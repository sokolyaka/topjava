...
import static ru.javawebinar.topjava.TestUtil.basic;
...
public class UserMealRestControllerTest extends AbstractControllerTest {
    ...
    public void testGet() throws Exception {
        mockMvc.perform(get(REST_URL + MEAL1_ID).with(basic(USER)))
    ...
    public void testDelete() throws Exception {
        mockMvc.perform(delete(REST_URL + MEAL1_ID).with(basic(USER)).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    ...
    public void testUpdate() throws Exception {
        ...
        mockMvc.perform(put(REST_URL + MEAL1_ID).with(basic(USER)).contentType(MediaType.APPLICATION_JSON)
    ...
    public void testCreate() throws Exception {
        ...
        ResultActions action = mockMvc.perform(post(REST_URL).with(basic(USER))
    ...
    public void testGetAll() throws Exception {
        mockMvc.perform(get(REST_URL).contentType(MediaType.APPLICATION_JSON).with(basic(USER)))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
    ...
    public void testGetBetween() throws Exception {
        mockMvc.perform(get(REST_URL + "between?startDateTime=2015-05-30T07:00&endDateTime=2015-05-31T11:00:00").with(basic(USER)))
                .andExpect(status().isOk())
                .andDo(print())
    ...
    public void testFilter() throws Exception {
        mockMvc.perform(get(REST_URL + "filter").with(basic(USER))
    ...
    public void testFilterAll() throws Exception {
        mockMvc.perform(get(REST_URL + "filter?startDate=&endTime=").with(basic(USER)))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(MATCHER_WITH_EXCEED.contentListMatcher(
